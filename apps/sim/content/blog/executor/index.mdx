...
There's a single guiding philosophy we use when designing the executor: workflows should read like the work you intend to do, not like the mess of cables behind the TV.
The complexity of wiring and plumbing should be abstracted away from the user, and building a performant workflow end to end should be easy, modular, and seamless.
...
### The Execution Queue: Event-Driven Execution
...
### Dependency Resolution: Constant Time Variable Resolution
...
### Variable Resolution: Scopes That Layer
...
### Executing from the Client: Seeing Execution as it Happens
...
## Loops: iteration that fits the work
...
## Conditions and Routers: deterministic branching
...
## Human in the loop: pause and resume anywhere
...
A typical pattern: run an agent in a loop, pause after each iteration for human feedback, use a router to decide whether to continue or exit based on that feedback, update a workflow variable with approval status, and let the while loop condition check that variable.

The executor handles this without custom glue. The loop iterates, the approval block pauses, you provide feedback via a resume link, the router evaluates it, the workflow variable updates, and the loop condition checks the variable. Each piece composes cleanly because pause/resume integrates with loops, parallels, routing, and variables at the executor level.
